...
| Concern | Status |
|---|---|
| Multi-module Maven build (parent + 3 children) | Done |
| Mock Authorization Server (Spring Authorization Server) | Done — alice/admin pre-registered, `spa-client` registered |
| BFF: oauth2Login, CSRF cookie repo, session config | Done |
| BFF: WebClient with `ServletOAuth2AuthorizedClientExchangeFilterFunction` | Done — proxy controllers don't touch tokens |
| BFF proxy controllers (accounts, transactions, users, admin) | Done |
| Resource Server: OAuth2 resource server with JWT issuer + audience validation | Done |
| `JwtAuthConverter` (creates user on first sign-in, maps role) | Done |
| Schema migrations + seed admin row | Done |
| JPA entities + repositories + DTOs + exceptions + global error handler | Done |
| Kafka producer config + `TransactionEventPublisher` | Done |
| `AccountService` ownership check (returns 404, not 403) | Done |
| `TransactionService` for DEPOSIT and WITHDRAWAL incl. insufficient-funds | Done |
| `PaymentService` (calls Payment Processor with API-key header) | Done |
| Frontend: Vite proxy, React Router, apiClient, useMe, AppLayout | Done |
| Frontend pages (Accounts, AccountDetail, NewTransaction, AdminUsers) | Done |
| WireMock stubs for the Payment Processor | Done |
